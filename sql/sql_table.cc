                                    ...
                                    uint key_count)
{
  ...
  while ((key=key_it++))
  {
    ...
    else if (key->period && key->type != Key::FOREIGN_KEY)
    {
      if (!create_info->period_info.is_set()
          || !key->period.streq(create_info->period_info.name))
      {
    ...
    {						// Field is changed
      ...
      if (mdl_ref_tables && (alter_info->flags & ALTER_RENAME_COLUMN) &&
          0 != my_strcasecmp(system_charset_info, def->change.str,
                             def->field_name.str))
      {
        if (table->s->foreign_keys.get(thd, refs_to_close, def->change, true))
        {
          my_error(ER_OUT_OF_RESOURCES, MYF(0));
          DBUG_RETURN(1);
        }
        if (table->s->referenced_keys.get(thd, refs_to_close, def->change, false))
        {
          my_error(ER_OUT_OF_RESOURCES, MYF(0));
          DBUG_RETURN(1);
        }
